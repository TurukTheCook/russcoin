    ...
    */
    readAll(req, res) {
      Message.find({ $or: [{ receiverId: res.locals.user._id }, { receiverId: res.locals.user.username}] }, (err, messages) => {
        ...
        else res.status(200).json({ success: true, message: 'Вот ваши сообщения! Here is your messages!', content: messages })
      })
    },
    ...
    */
    update(req, res) {
      Message.findById(req.params.messageID,  (err, message) => {
        ...
        else if (message.receiverId != res.locals.user._id && message.receiverId != res.locals.user.username) res.status(403).json({ success: false, message: 'CYKA BLYAT !' })          
